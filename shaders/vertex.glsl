// Particle portrait vertex shader
// Noise functions (snoise, fbm, turbulence, curlNoise, curlNoiseFbm, windField)
// are prepended from utils/noise.ts via ParticleMaterial.
...
// `position` attribute = portrait origin (aOrigin)
...
// ---------------------------------------------------------------------------
// Easing (cinematic, non-linear assembly)
// ---------------------------------------------------------------------------
...
// Blend easeOutExpo with slight overshoot for a cinematic settle.
...
// ---------------------------------------------------------------------------
// Idle: breathing, floating, mouse parallax
// ---------------------------------------------------------------------------
vec3 applyIdleMotion(vec3 pos, float seed) {
  ...
  // Mouse parallax: stronger toward edges of portrait.
...
// ---------------------------------------------------------------------------
// Dissolve: Infinity War dust — curl, turbulence, wind, stagger, swirl
// ---------------------------------------------------------------------------
vec3 applyDissolve(vec3 pos, float seed, out float dissolveFade) {
  ...
  // Per-particle stagger so dissolution ripples across the portrait.
  ...
  // Velocity-driven scatter with per-particle speed variance.
  ...
  // Quadratic ramp for accelerating separation.
  dissolveOffset *= dActive * dActive * (1.8 + seed * 2.5);

  // Swirl / slight rotation during dissolve.
  ...
  // Upward drift like ash lifting on wind.
